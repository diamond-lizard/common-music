...
CodeEditorWindow::CodeEditorWindow (juce::File file, juce::String text, int synt, juce::String title)
  : juce::DocumentWindow(juce::String::empty, juce::Colours::white, juce::DocumentWindow::allButtons, true)
{
...
void CodeEditorWindow::setWindowTitle(juce::String title)
{
  if (title.isEmpty())
  {
    if (sourcefile == juce::File::nonexistent)
    {
...
void CodeEditorWindow::openFile(juce::File file)
{
  if (file==juce::File::nonexistent || file.isDirectory())
  {
    juce::FileChooser choose ("Open File",
                              (file==juce::File::nonexistent) ? juce::File::getCurrentWorkingDirectory() : file,
                              juce::String::empty, true);
...
{
  new CodeEditorWindow(juce::File::nonexistent, content, synt, title);
}
...
CodeEditorWindow* CodeEditorWindow::getFocusCodeEditor()
{
  for (int i=0; i<juce::TopLevelWindow::getNumTopLevelWindows(); i++)
  {
    ...
    if (s[ juce::var::identifier("FocusEditor") ]==juce::var(true))
...
FindAndReplaceDialog::FindAndReplaceDialog ()
  : label1 (0),
    ...
    textButton5 (0)
{
  addAndMakeVisible (label1 = new juce::Label (juce::String::empty, "Find:"));
  ...
  addAndMakeVisible (textEditor1 = new juce::TextEditor (juce::String::empty));
  ...
  textEditor1->setText (juce::String::empty);
  textEditor1->setWantsKeyboardFocus(true);
  textEditor1->addListener(this);

  addAndMakeVisible (label2 = new juce::Label (juce::String::empty, "Replace:"));
  ...
  addAndMakeVisible (textEditor2 = new juce::TextEditor (juce::String::empty));
  ...
  textEditor2->setText (juce::String::empty);
  textEditor2->addListener(this);
  
  addAndMakeVisible (textButton1 = new juce::TextButton (juce::String::empty));
  ...
  addAndMakeVisible (textButton2 = new juce::TextButton (juce::String::empty));
  ...
  addAndMakeVisible (textButton3 = new juce::TextButton (juce::String::empty));
  ...
  addAndMakeVisible (textButton4 = new juce::TextButton (juce::String::empty));
  ...
  addAndMakeVisible (textButton5 = new juce::TextButton (juce::String::empty));
    ...
    regionState(juce::Range<bool>(false, false))
{
  ...
  juce::Font mono ((SysInfo::isMac()) ? "Monaco" : juce::Font::getDefaultMonospacedFontName(),
...
void CodeEditor::focusGained(juce::Component::FocusChangeType cause)
{
  ...
  for (int i=0; i<juce::TopLevelWindow::getNumTopLevelWindows(); i++)
  {
    ...
    juce::NamedValueSet& s=w->getProperties();
    if (s.contains(juce::var::identifier("FocusEditor")))
      s.set(juce::var::identifier("FocusEditor"), juce::var(false)) ;
    // FIXME
  ...
  getTopLevelComponent()->getProperties().set(juce::var::identifier("FocusEditor"), juce::var(true));
}
...
void CodeEditor::killWordForward()
{
  if (getCaretPos()!=getEOB())
  {
    ...
    insertTextAtCaret (juce::String::empty);
    isChanged(true);
  }
}
